...
function iapOrder(id){
    ...
    store.when(id).approved(function (product) {
        ...
        $.post("https://femytalk.com/setheartplus.php",{
            p:my,
            heart:heart
        },function(d,e){
            halert("결제가 완료되었습니다.");    
...
function file_upload(){
    ...
    ft.upload(file_path, "https://femytalk.com/upload.php", 
        function(response) {
            // HTTP 결과코드 받기
...
function getmsg(e){
	...
	if(r.msg=="photo"){
        ...
        $("#ifr").attr("src","https://femytalk.com/chat.php?id="+r.id+"&p="+my);
    }
        ...
        $("#ifr").attr("src","https://femytalk.com/board_detail.php?id="+r.id+"&p="+my);
    }
    else if(r.msg=="go-board-create"){
        $("#ifr").attr("src","https://femytalk.com/board_detail.php?p="+my);
    }
        ...
        $("#ifr").attr("src","https://femytalk.com/index.php");   
    }
...
function invite(chatid){
    $("#invite").modal('hide');
    $("#ifr").attr("src","https://femytalk.com/chat.php?id="+chatid+"&p="+my);    
}
	...
	window.plugins.sim.requestReadPermission((r)=>{
		window.plugins.sim.getSimInfo((r)=>{
			...
			var msg="{\"phone\":\""+my+"\"}";
 			document.getElementById("ifr").contentWindow.postMessage(msg,"*");
            clearInterval(gn);
            $("#intro").animate({opacity:'0'},"slow",function(){ $("#intro").remove(); alert(my);});
		}, (e)=>{
            alert(e);
        });
	}, (e)=>{
...
function chatexit(){
    ...
    $("#chat").modal('hide');
    //$("#ifr").attr("src","http://femytalk.com/index.php");
    var msg="{\"msg\":\"back\"}";
    document.getElementById("ifr").contentWindow.postMessage(msg,"*");   
}
